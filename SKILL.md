...
Use `references/sdr-template.md`.
...
Read `references/storage-and-stack-selection.md` before making the decision and document the choice in an SDR.
...
Prefer GitHub Pages deployment through a custom GitHub Actions workflow. Read `references/github-pages-deployment.md` and use one of the templates in `assets/`.

Choose the template that matches the implementation:
- `assets/github-pages-static.yml` for plain static apps with no build step
- `assets/github-pages-node.yml` for build-based apps such as Vite
